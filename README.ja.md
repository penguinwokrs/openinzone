...
インストーラーは最初に言語を尋ねます。英語・日本語・簡体字中国語から選べ、Windows 自体の言語から
自動的に選ばれています。ここで選んだ言語は次のページのチェックボックスにも、インストール後の
トレイアプリのメニューにも引き継がれます。あとから設定ウィンドウで切り替えることもできます。
[ホットキーと設定](#ホットキーと設定)を参照してください。
...
`inzone` にも PATH が通ります。更新はどちらの方法でもできます。**設定**の更新ボタンは Scoop を
使って更新します（PowerShell のウィンドウが開き、終わると自動で閉じます）。`scoop update openinzone`
はトレイを起動したままでも実行でき、トレイは 15 秒以内に気づいて新しい版で起動し直します。更新を
止めてしまうのは、ターミナルで動かしたままの `inzone watch` だけです。[困ったときは](#困ったときは)
を参照してください。Stream Deck プラグインは含まれません。[インストール](#インストール-1)を参照して
ください。
...
PC からマイクミュートを切り替えられるのは、INZONE Hub と同じく INZONE Buds だけです。ヘッドセットは
本体のボタンでミュートするので、このアイコンと `inzone mic mute` は何も送らずに断ります。Stream Deck の
マイクミュートキーは、ヘッドセットではマイクレベルでミュートします。押すとレベルを 0 に、もう一度
押すとキーの **Unmute level**（空欄なら 100）に戻します。本体のボタンかレベル 0 のどちらかでミュート
されていれば `MUTED` と表示します。
...
| 外音コントロールを切り替え（オフ → ノイズキャンセリング → 外音取り込み） | `Ctrl+Alt+Shift+A` |

右クリックメニューの**設定**を選ぶと、**全般・デバイス・ホットキー・アップデート・プラグイン**の
5 つのタブを持つウィンドウが開きます。保存ボタンはありません。変えたその場で反映されます。

**ホットキー**タブには 9 つすべてと、それぞれに割り当たっているキーが並びます。行を選んで組み
...
**全般**タブにはチェックボックスが 2 つと、その下に**表示言語**の選択があります。**Windows の
起動時に常駐する**は、その名のとおりトレイを Windows と一緒に起動します。**起動時に更新を確認
する**は、ログインのたびに 1 回だけ、ログインの邪魔にならないよう約 30 秒待ってから、新しい
リリースがあるかを GitHub に問い合わせます。あるときだけ通知するので、なければ何も出ません。
通知をクリックすると、下の**アップデート**タブが開いた状態で設定ウィンドウが表示され、ボタンは
すでに**更新**と表示されています。既定はオフです。**表示言語**は English・日本語・简体中文
から、それぞれの言語自身の表記で選べます。切り替えるとトレイの再起動を提案されますが、次回起動
まで待つこともできます。
...
いたレベルが一緒に返ってきます。ヘッドセットが接続されていない間はタブ全体が無効になります。

**お使いのモデルが持っていない設定は、オフとして表示するのではなく、そもそも表示しません。**
どれを持っているかは推測していません。ヘッドセットは自分が何を持っているかの一覧を自ら公開して
おり、接続のたびに 1 回それを読んでいます。パネルと Stream Deck プラグインも同じ答えを見るので、
ゲーム/チャットバランスを持たないモデルにバランスのスライダーやキーが出ることはありません。

これらは INZONE Hub の通信をパケット単位で観察して解読したものです。何を送っているかは
[docs/PROTOCOL.md](docs/PROTOCOL.md) に、何を持っているかをどう答えるかは
[ケイパビリティマップ](docs/PROTOCOL.md#the-headset-publishes-its-own-capability-map-0x060x08)
にあります。
...
Elgato Stream Deck 用のプラグインがあります。12 個あるアクションのうち 6 個は、操作している値を
キーに表示します。左右それぞれのバッテリー、バランスの数値、マイクがミュートかどうか、そして
外音コントロールの現在モードです。トレイのパネルやヘッドセット側で変えても、キーの表示が即座に
追随します。残る 6 個は上げ下げの向きが決まった専用キーで、こちらは数値表示ではなく絵です。
押すとそのときの値が少しの間だけ表示され、また絵に戻ります。Stream Deck + ならどのアクションも
ダイヤルに割り当てられ、連続値にはこちらのほうが向いています。
...
| 音量アップ | 設定した幅で上げる | 回して調整、押して上げる | 押すと少しの間 `16 / 30` |
| 音量ダウン | 設定した幅で下げる | 回して調整、押して下げる | 押すと少しの間 `16 / 30` |
| ゲーム / チャットバランス | 設定した幅で増減 | 回して調整、押して中央へ | `GAME 1.0` / `CENTRE` / `CHAT 2.0` |
| ゲーム寄りへ | 設定した幅でゲーム側へ | 回して調整、押して動かす | 押すと少しの間 `GAME 1.0` |
| チャット寄りへ | 設定した幅でチャット側へ | 回して調整、押して動かす | 押すと少しの間 `CHAT 2.0` |
| マイクミュート | 切り替え | 押して切り替え | `MUTED` / `LIVE` |
| マイクレベル | 設定した幅で増減 | 回して調整、押してミュート | `75 %` |
| マイクレベルアップ | 設定した幅で上げる | 回して調整、押して上げる | 押すと少しの間 `75 %` |
| マイクレベルダウン | 設定した幅で下げる | 回して調整、押して下げる | 押すと少しの間 `75 %` |
| 外音コントロール | オフ・ノイズキャンセリング・外音取り込みの順に切り替え | 回して切り替え、押して次のモード | `OFF` / `NC` / `AMB` |
...
音量・バランス・マイクレベルの素のアクションは、**キー**では step の符号で向きが決まります。負の値にすると下げるキーになるので、2 つ並べれば上げ下げが揃います。ダイヤルでは符号は無視され、回した向きがそのまま方向になります。向きが決まっている 6 つのアクションはそれ自体が矢印つきの絵として向きを持っているので、パネルは入力値を絶対値にしてそのままフィールドへ書き戻します。**音量ダウン**のキーが設定次第で音量を上げてしまうことはありません。これらは数値表示ではなく絵で、押すと現在値が少しの間だけ出て、そのあとまた絵に戻ります。
...
Stream Deck が取り込むかどうかを一度尋ね、**OpenInzone** の下に 12 個のアクションが現れます。
...
**`scoop update openinzone` で `The following instances of "openinzone" are still running.` と出る**
Scoop の OpenInzone 用フォルダーからプログラムが動いています。ほとんどの場合、別のターミナルで
動いている `inzone watch` です（起動時にその旨を表示します）。Ctrl+C で止めてから、もう一度更新
してください。1.1.3 まではトレイと daemon も原因になります。**設定**の更新ボタンを使うか、先に
トレイをメニューから終了してください。
...
`tools/ShowSettings` は、トレイと同じ手順で本物の設定ウィンドウを組み立て、動いている daemon に
つないだうえで、各タブの画像を保存します。
...
そもそも開けないウィンドウがリリースに乗ってしまったことがあり、そのために用意しました。マーク
アップに書いたハンドラーはウィンドウの読み込み中に実行され、そのうちの 1 つがまだ存在しないラベル
に触れていました。どちらの検査にもかかりませんでした——テストはウィンドウを構築せず、スクリーン
ショット用スクリプトは（コード側を解決できないので）ハンドラーを剥がしてから読みます。マークアッ
プとロジックはそれぞれ覆われていて、両者を組み合わせた瞬間だけが抜けていたわけです。ここで動かす
のはその組み合わせです。

実機から実際のチャネル越しに値を受け取るので、「人に見えているもの」が「デバイスが答えたもの」か
どうかも分かります。`--late-start` はチャネルが上がる前にウィンドウを開きます（daemon が起動中の
場合に起きること）。ウィンドウが要求をやり直すかどうかを、仮定ではなく確認できます。
...
リポジトリ側もそれに合わせてあります。どれもファイルとして残らない設定なので、ここに書いておきます。

| 設定 | 値 | 理由 |
|---|---|---|
| マージ方法 | squash のみ | プルリクエスト 1 つにつきコミット 1 つ。履歴がノートと同じ読み口になる |
| squash のタイトル | プルリクエストのタイトル | 既定のままだと 1 コミットの PR はコミット件名で squash され、履歴とノートが食い違う |
| squash の本文 | 各コミットのメッセージ | 「なぜそうしたか」はコミット本文に書いているため。PR 本文で置き換えると消える |
| マージ後にブランチを削除 | する | |
| `main` のルールセット | プルリクエスト必須・承認 0・squash のみ・削除禁止・強制 push 禁止 | 何かが強制しない限り、運用ルールは習慣どまりになる |

`main` への直接 push は、リポジトリの所有者からのものでも拒否されます。邪魔になったときは
**Settings → Rules** から外せます。
...
src/OpenInzone.Resources  3 言語ぶんのウィンドウ・トレイ・フライアウトのテキスト。独立したアセン
                          ブリなのは、この SDK では WPF が同一アセンブリ内の resx 生成クラスを
                          XAML から解決できないためで、OpenInzone.Tray のマークアップコンパイラ
                          から参照できる場所に置く必要がある
...
installer/lang/           Inno Setup が同梱していない .isl。簡体字中国語を外部から取り込んでいる
...
Visual Studio や Rider 用に、11 のプロジェクトを `OpenInzone.sln` がまとめています。
