...
Elgato Stream Deck 用のプラグインがあります。11 個あるアクションのうち 5 個は、ホットキーにできないこと、つまり**操作している値をキーに表示する**キーです。左右それぞれのバッテリー、バランスの数値、マイクがミュートかどうか。どこで変わっても即座に反映されるので、トレイのパネルで動かしても、イヤホン側で操作しても、キーの表示が追随します。残る 6 個は上げ下げの向きが決まった専用キーで、こちらは数値表示ではなく絵です。押すとそのときの値が少しの間だけ表示され、また絵に戻ります。Stream Deck + ならどのアクションもダイヤルに割り当てられ、連続値にはこちらのほうが向いています。
