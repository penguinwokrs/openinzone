...
押すとそのときの値が少しの間だけ表示され、また絵に戻ります。Stream Deck + では ANC 以外の
アクションをダイヤルにも割り当てられ、連続値にはこちらのほうが向いています。
...
| ANC | オフ -> ANC -> 外音取り込みの順に切り替え | 利用不可 | `OFF` / `ANC` / `PASS` |
| バッテリー | 押して再読み取り | 押して再読み取り | `L 97` と `R 94` |

ANC はキー専用です。切り替え順は固定で、キーにはプラグイン内の推測ではなくヘッドセットが返した
現在モードが表示されます。
...
ANC はキーに、それ以外はキーまたは Stream Deck + のダイヤルにドラッグしてください。
