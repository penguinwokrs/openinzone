...
public class TextRendererTests
{
    private static string Render(IReport report, bool raw = false)
    {
        ...
        new TextRenderer(writer, raw).Render(report);
...
/// <summary>The aligned columns the tool has always printed.</summary>
public sealed class TextRenderer(TextWriter output, bool raw = false) : IReportRenderer
{
...
```csharp
    private static int Main(string[] args)
    {
        ...
        IReportRenderer renderer = new TextRenderer(Console.Out);
...
```csharp
    private static int Main(string[] args)
    {
        ...
        IReportRenderer renderer = json
            ...
            : new TextRenderer(Console.Out, raw);
...
The catch block now renders through the renderer, so `--json` produces an object on the error path too. Errors move from stderr to stdout as a result: a consumer reading stdout gets exactly one object either way.
