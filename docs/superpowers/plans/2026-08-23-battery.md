...
- [ ] **Step 4: Do not open a device to reject a command that does not exist**

`Run` opens the dongle before the switch decides anything, so `inzone nonsense` on a machine with
no dongle plugged in fails with a device error and exit 1 — never reaching `Unknown` and its exit
2. Rejecting a command that does not exist should not need hardware.

Move the open inside the arms that need it, so the switch stays the single list of commands:
...
`inzone watch batery` still opens a device before it rejects the filter word, because `watch`
itself is a real command. Leave that as it is — it is a device command, and reporting a missing
dongle first is defensible. Note it in your report rather than restructuring further.

- [ ] **Step 5: Verify by hand**

Two of these need no dongle at all now, which is the point of Step 4:

```
inzone nonsense       ; echo $?    → 2, with no dongle attached
inzone --help         ; echo $?    → 0
```

The rest need the hardware:
...
- [ ] **Step 3: Teach `inzone --help` the new surface**

`PrintUsage` in `src/OpenInzone.Cli/Program.cs` is what someone sees when they run `inzone` with
no arguments, and it still describes only the commands that existed before this work. Extend the
raw string literal so the built-in help and the README agree.

After the existing `inzone watch` line, add:
...
Keep the indentation of the surrounding raw string literal — the existing entries are indented to
line up under `Usage:`, and a mismatch will show up in the output.

While you are in this method, delete the stray double blank line left after the filter guard in
`Watch` (around `src/OpenInzone.Cli/Program.cs:169`). It came in from the plan's own sample.

- [ ] **Step 4: Document the new command surface in English**
...
- [ ] **Step 5: Document it in Japanese**
...
- [ ] **Step 6: Say that the case reading is a snapshot**
